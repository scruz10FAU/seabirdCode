...
class SimCamera(CameraInterface, Node):
    ...
    def open(self) -> bool:
        ...
        self._info_sub = self.create_subscription(
            CameraInfo,
            f"{self._topic_prefix}/camera_info",
        ...
        rgb_sub = message_filters.Subscriber(
            self, Image, f"{self._topic_prefix}/rgb", qos_profile=qos
        )
        depth_sub = message_filters.Subscriber(
            self, Image, f"{self._topic_prefix}/depth", qos_profile=qos
        )
...
def main():
    ...
    YOLO_WEIGHTS = os.path.expanduser(
        "~/seabird_dataset/runs/v1/weights/best.pt"
    )
